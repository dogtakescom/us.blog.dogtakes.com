...
fetch(url,requestData).then(response=>{if(!settings.form_webhook.includes('static-studio')){if(response.ok){handleMessage(settings)}}}).catch(error=>{if(!settings.form_webhook.includes('static-studio')){if(error.message.includes('Failed to fetch')){handleMessage(settings,!1)}else{handleMessage(settings,!0)}}})}
...
document.addEventListener("DOMContentLoaded",function(){const allForms=document.querySelectorAll(".wpcf7 form, .wpcf7-form, .gform_wrapper form, .wpforms-container form, .elementor-form, .wsf-form, .frm-fluent-form, .brxe-form, .brxe-brf-pro-forms");allForms.forEach((form)=>{modifyFormAttributes(form);const inputs=form.querySelectorAll("input");inputs.forEach((input)=>{if(input.getAttribute("aria-required")==="true"){input.required=!0}});if(typeof gform!="undefined"){gform.utils.addAsyncFilter('gform/submission/pre_submission',async(data)=>{data.abort=!0;form.dispatchEvent(new Event('submit'));return data})}
